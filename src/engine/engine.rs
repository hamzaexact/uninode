...
impl Engine {
    ...
    pub fn spawn_arrow(&mut self, mut arrow_node: NodeId, row: Option<usize>, height: Option<usize>, col: Option<usize>, other_height: Option<usize>)  -> Cell {
    ...
    pub fn spawn_at_left(&mut self, original_node: &Cell, current_node: NodeId, arrow_id: NodeId,) -> Cell {
        ...
        if let RenderedNode::Box(box_2) = borrowed_node2 {
            if let NodeId::ArrowId(ref ref_arrow) = arrow_id {
                (*box_2).col = Some(original_node.col + original_node.length + ref_arrow.length.unwrap()
                    + if ref_arrow.has_head {1} else {0} + if ref_arrow.with_offset {2} else{0});
                (*box_2).row = Some(original_node.row);
                node2.row = original_node.row;
                node2.col = original_node.col + original_node.length + ref_arrow.length.unwrap() + if ref_arrow.has_head {1} else {0} + if ref_arrow.with_offset {2} else{0};;
            }
        }
        let arrow_cell = self.spawn_arrow(arrow_id, Some(original_node.row), Some(original_node.height), Some(original_node.col + original_node.length), Some(node2.height));
        node2
    }
    // pub fn left_spawn()
